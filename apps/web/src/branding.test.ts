...
describe("branding", () => {
  it("uses injected desktop branding when available", async () => {
      ...
      value: {
        desktopBridge: {
          getAppBranding: () => ({
            baseName: "Sigma Code",
            stageLabel: "Nightly",
            displayName: "Sigma Code (Nightly)",
          }),
        },
      },
    });
    ...
    expect(branding.APP_BASE_NAME).toBe("Sigma Code");
    expect(branding.APP_STAGE_LABEL).toBe("Nightly");
    expect(branding.APP_DISPLAY_NAME).toBe("Sigma Code (Nightly)");
  });
  ...
  it("does not label the latest hosted app channel", async () => {
    ...
    expect(branding.APP_DISPLAY_NAME).toBe("Sigma Code");
  });
...
describe("branding logic", () => {
  ...
  it("updates the display name for nightly primary server versions", () => {
    expect(
      resolveServerBackedAppDisplayName({
        baseName: "Sigma Code",
        fallbackDisplayName: "Sigma Code (Alpha)",
        fallbackStageLabel: "Alpha",
        primaryServerVersion: "0.0.28-nightly.20260616.12",
      }),
    ).toBe("Sigma Code (Nightly)");
  });
