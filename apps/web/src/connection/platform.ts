...
function clientMetadata() {
  ...
  return {
    label: desktop ? "Sigma Code Desktop" : "Sigma Code Web",
    ...
    const cloudSession = CloudSession.of({
      clerkToken: Effect.gen(function* () {
        ...
        if (session === null) {
          return yield* new ConnectionBlockedError({
            reason: "authentication",
            detail: "Sign in to Sigma Connect to connect this environment.",
          });
        }
        ...
        if (token === null) {
          return yield* new ConnectionBlockedError({
            reason: "authentication",
            detail: "The Sigma Connect session is unavailable.",
          });
        }
        return token;
      }),
