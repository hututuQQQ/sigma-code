...
/**
 * Drives the primary environment's Sigma Connect link. Sigma Connect (managed
...
export function useCloudLinkController() {
  ...
  const reportUpdateFailure = (cause: unknown) => {
    ...
    toastManager.add({
      type: "error",
      title: "Could not update Sigma Connect",
      description: message,
      data: traceId
        ? {
  ...
  const reconcileCloudState = async (desired: CloudLinkDesiredState): Promise<boolean> => {
    ...
    if (!wantsLink) {
      ...
      // leave the user unable to turn Sigma Connect off.
      ...
      if (!clerkToken) {
        reportUpdateFailure(new Error("Sign in to Sigma Connect before enabling this."));
        return false;
      }
