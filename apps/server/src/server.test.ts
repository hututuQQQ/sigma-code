    ...
    const appLayer = servedRoutesLayer.pipe(
      ...
      Layer.provide(
        Layer.mock(CloudCliTokenManager.CloudCliTokenManager)({
          get: Effect.die(new Error("Unexpected Sigma Connect CLI authorization request.")),
...
const makeCloudMintCredentialRequest = (input: {
  ...
  const payload = {
    ...
    aud: input.audience ?? `sigmacode-env:${input.environmentId}`,
...
it.layer(NodeServices.layer)("server router seam", (it) => {
  ...
  it.effect("serves the public environment descriptor without requiring auth", () =>
    Effect.gen(function* () {
      ...
      const url = yield* getHttpServerUrl("/.well-known/sigmacode/environment");
  ...
  it.effect("serves the documented Sigma Connect mint credential endpoint", () =>
    Effect.gen(function* () {
      yield* buildAppUnderTest();
      ...
      const mintUrl = yield* getHttpServerUrl("/api/sigma-connect/mint-credential");
  ...
  it.effect("serves signed Sigma Connect environment health checks", () =>
    Effect.gen(function* () {
      yield* buildAppUnderTest();
      ...
      const healthUrl = yield* getHttpServerUrl("/api/sigma-connect/health");
    ...
    Effect.gen(function* () {
      ...
      const wrongAudience = yield* postMint(
        makeCloudMintCredentialRequest({
          ...
          environmentId: testEnvironmentDescriptor.environmentId,
          clientProofKeyThumbprint: "client-proof-key-thumbprint",
          audience: "sigmacode-env:other-environment",
  ...
  for (const desktopOrigin of ["sigmacode://app", "sigmacode-dev://app"]) {
  ...
  it.effect("routes websocket rpc subscribeServerConfig streams snapshot then update", () =>
    Effect.gen(function* () {
      ...
      if (first?.type === "snapshot") {
        ...
        assert.match(first.config.observability.logsDirectoryPath, /[/\\]logs$/u);
  ...
  it.effect("preserves structured workspace rpc failures", () =>
    Effect.gen(function* () {
      ...
      if (platform === "win32") {
        // Creating a file symlink requires Developer Mode or elevation on
        // Windows. The path-escape behavior is covered on platforms where the
        // host can construct the fixture.
        return;
      }
      const fs = yield* FileSystem.FileSystem;
      const path = yield* Path.Path;
