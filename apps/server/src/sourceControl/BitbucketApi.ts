...
const BitbucketApiEnvConfig = Config.all({
  baseUrl: Config.string("SIGMACODE_BITBUCKET_API_BASE_URL").pipe(
    Config.withDefault(DEFAULT_API_BASE_URL),
  ),
  accessToken: Config.string("SIGMACODE_BITBUCKET_ACCESS_TOKEN").pipe(Config.option),
  email: Config.string("SIGMACODE_BITBUCKET_EMAIL").pipe(Config.option),
  apiToken: Config.string("SIGMACODE_BITBUCKET_API_TOKEN").pipe(Config.option),
});
...
function checkoutBranchName(input: {
  ...
  return `sigmacode/pr-${input.pullRequestId}/${sanitizeBranchFragment(input.headBranch)}`;
}
...
function authFromConfig(
  ...
  return {
    ...
    host: Option.some("bitbucket.org"),
    detail: Option.some(
      "Set SIGMACODE_BITBUCKET_EMAIL and SIGMACODE_BITBUCKET_API_TOKEN, or SIGMACODE_BITBUCKET_ACCESS_TOKEN.",
    ),
  };
}
