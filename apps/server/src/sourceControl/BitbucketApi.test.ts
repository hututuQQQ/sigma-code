    ...
    Layer.provide(
      ConfigProvider.layer(
        ConfigProvider.fromEnv({
          env: {
            SIGMACODE_BITBUCKET_API_BASE_URL: "https://api.test.local/2.0",
            SIGMACODE_BITBUCKET_EMAIL: "user@example.com",
            SIGMACODE_BITBUCKET_API_TOKEN: "token",
          },
        }),
      ),
    ),
...
it.effect("checks out fork pull requests through an ensured fork remote", () => {
  ...
  return Effect.gen(function* () {
    ...
    assert.deepStrictEqual(git.fetchRemoteBranch.mock.calls[0]?.[0], {
      ...
      remoteName: "octocat",
      remoteBranch: "main",
      localBranch: "sigmacode/pr-42/main",
    });
    assert.deepStrictEqual(git.setBranchUpstream.mock.calls[0]?.[0], {
      cwd: "/repo",
      branch: "sigmacode/pr-42/main",
      remoteName: "octocat",
      remoteBranch: "main",
    });
    assert.deepStrictEqual(git.switchRef.mock.calls[0]?.[0], {
      cwd: "/repo",
      refName: "sigmacode/pr-42/main",
    });
  }).pipe(Effect.provide(layer));
});
