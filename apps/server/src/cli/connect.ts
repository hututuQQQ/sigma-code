...
const authorizeCli = Effect.fn("cloud.cli.authorize")(function* (options: {
  ...
  const existing = yield* tokens.getExisting.pipe(
    Effect.catchTag("CloudCliCredentialRefreshError", () =>
      Console.log(
        "The stored Sigma Connect credential could not be refreshed; signing in again.",
...
function formatCloudStatus(status: CloudCliStatus, options?: { readonly json?: boolean }): string {
  ...
  const nextStep = !status.authenticated
    ? "Run `sigma-code connect link` to authorize and enable Sigma Connect."
    : !status.desired
      ? "Run `sigma-code connect link` to enable Sigma Connect."
      : !status.linked
        ? "Start Sigma Code to provision the environment link and launch its managed tunnel."
        : undefined;
...
const confirmRelayClientInstall = (version: string) =>
  Prompt.run(
    Prompt.confirm({
      message: `The Sigma relay client is required for Sigma Connect. Download and install version ${version}?`,
      initial: false,
    }),
  );
...
const logCloudDisconnectFailure = (
  ...
  Effect.logWarning("Sigma Connect disconnect operation failed.").pipe(
    Effect.annotateLogs({
      operation,
      clearAuthorization,
...
export const reportCloudDisconnectResults = Effect.fn("cloud.cli.report_disconnect_results")(
  function* (input: {
    ...
    if (input.liveResult.status === "failed") {
      ...
      yield* Console.warn(
        "Sigma Connect is disabled, but the running server could not stop its tunnel.\nRestart that server to stop the connector.",
      );
    } else {
      yield* Console.log("Sigma Connect is disabled locally.");
    }
    ...
    if (Exit.isFailure(input.relayResult)) {
      ...
      yield* Console.warn(
        input.clearAuthorization
          ...
          : "Could not revoke the relay-side environment record yet.\nRun `sigma-code connect unlink` again when the relay is reachable.",
      );
...
const disconnectCloud = Effect.fn("cloud.cli.disconnect")(function* (options: {
  ...
  if (options.clearAuthorization) {
    yield* Console.log(
      "Signed out of Sigma Connect locally.\nThe background service is managed separately with `sigma-code service`.",
    );
  }
});
...
const linkEnvironmentForConnect = Effect.fn("cloud.cli.link_environment")(function* (options: {
  ...
  if (!publishOnly) {
    ...
    if (Option.isNone(installed)) {
      yield* Console.log("Sigma Connect setup cancelled. The relay client was not installed.");
      return null;
    }
...
const connectLoginCommand = Command.make("login", {
  ...
  Command.withDescription("Authorize the Sigma Connect CLI without enabling remote access."),
  Command.withHandler((flags) =>
    runCloudCommand(
      flags,
      Effect.gen(function* () {
        yield* Console.log("Sigma Connect\n");
...
const connectLinkCommand = Command.make("link", {
  ...
  Command.withDescription("Authorize this environment for Sigma Connect on next start."),
  Command.withHandler((flags) =>
    runCloudCommand(
      flags,
      Effect.gen(function* () {
        ...
        if (linked) {
          yield* Console.log(
            flags.publishOnly
              ? `✓ Authorized${connectedAs(linked.identity)}\n\nNext\n  Start Sigma Code to publish agent activity (no managed tunnel).`
              : `✓ Authorized${connectedAs(linked.identity)}\n\nNext\n  Start the server with \`sigma-code serve\` to make this machine reachable.`,
          );
        }
      }),
    ),
...
const connectStatusCommand = Command.make("status", {
  ...
  Command.withDescription("Show persisted Sigma Connect and relay client state."),
  Command.withHandler((flags) =>
    runCloudCommand(
      flags,
      Effect.gen(function* () {
      ...
      Effect.gen(function* () {
        ...
        if (!enabled) {
          ...
          if (!linkedNow && (yield* CliState.readCliDesiredLinkMode) === "publish_only") {
            yield* CliState.setCliDesiredCloudLink(false);
            yield* Console.log("Cancelled the pending publish-only Sigma Connect link.");
          }
        ...
        // out of band without Sigma Connect.
        if (!(yield* tokens.hasCredential)) {
          yield* Console.log(
            "Run `sigma-code connect login` first so this environment can be authorized to publish.",
          );
          return;
        }
        // A link may already be desired (e.g. `sigma-code connect link` before the
        ...
        if (yield* CliState.readCliDesiredCloudLink) {
          yield* Console.log(
            "A Sigma Connect link is already pending. Start Sigma Code to finish provisioning it; publishing starts once it links.",
          );
          return;
        }
        yield* CliState.setCliDesiredCloudLink(true, "publish_only");
        yield* Console.log(
          "Restart Sigma Code to finish authorizing this environment to publish (no managed tunnel is created).",
        );
      }),
    ),
  ),
...
const connectUnlinkCommand = Command.make("unlink", {
  ...
  Command.withDescription("Disable Sigma Connect while retaining the stored authorization."),
  Command.withHandler((flags) =>
...
const connectLogoutCommand = Command.make("logout", {
  ...
  Command.withDescription("Disable Sigma Connect and clear the stored CLI authorization."),
  Command.withHandler((flags) =>
...
export const connectCommand = Command.make("connect", {
  ...
  Command.withDescription("Set up Sigma Connect for this machine."),
  Command.withHandler((flags) =>
    runCloudCommand(
      flags,
      Effect.gen(function* () {
        ...
        yield* Console.log(
          background
            ? "\n✓ Background service ready\n\nSigma Code will stay reachable after you log out."
            : "\nNext\n  Start the server with `sigma-code serve` to make this machine reachable.",
        );
      }),
    ),
