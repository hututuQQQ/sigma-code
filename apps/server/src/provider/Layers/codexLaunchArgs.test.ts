...
describe("resolveCodexLaunchArgs", () => {
  it("uses SIGMACODE_CODEX_LAUNCH_ARGS before configured settings", () => {
    NodeAssert.equal(
      resolveCodexLaunchArgs(" --strict-config ", {
        SIGMACODE_CODEX_LAUNCH_ARGS: "--enable foo",
      }),
      "--enable foo",
    );
  });

  it("uses configured settings when SIGMACODE_CODEX_LAUNCH_ARGS is empty", () => {
    NodeAssert.equal(
      resolveCodexLaunchArgs(" --strict-config ", { SIGMACODE_CODEX_LAUNCH_ARGS: "   " }),
      "--strict-config",
    );
  });

  it("ignores whitespace-only environment values", () => {
    NodeAssert.equal(resolveCodexLaunchArgs("", { SIGMACODE_CODEX_LAUNCH_ARGS: "   " }), "");
  });
});
