 ...
 *     (`codex`, `claudeAgent`, `cursor`, `grok`, `sigma`, `opencode`) in a single
 ...
 * without trying to spawn real `codex` / `claude` / `agent` / `grok` / `sigma` / `opencode`
...
import { SigmaDriver } from "../Drivers/SigmaDriver.ts";
...
const makeSigmaConfig = (overrides: Partial<SigmaSettings>): SigmaSettings => ({
  enabled: false,
  binaryPath: "sigma",
  customModels: [],
  ...overrides,
...
describe("ProviderInstanceRegistryLive — all drivers slice", () => {
  ...
  it.live("boots one instance of every shipped driver from a single config map", () =>
    Effect.gen(function* () {
      ...
      const sigmaId = ProviderInstanceId.make("sigma_default");
      ...
      const sigmaDriverKind = ProviderDriverKind.make("sigma");
      ...
      const configMap: ProviderInstanceConfigMap = {
        ...
        [sigmaId]: {
          driver: sigmaDriverKind,
          displayName: "Sigma",
          enabled: false,
          config: makeSigmaConfig({}),
        },
        [openCodeId]: {
      ...
      const { registry } = yield* makeProviderInstanceRegistry({
        drivers: [CodexDriver, ClaudeDriver, CursorDriver, GrokDriver, SigmaDriver, OpenCodeDriver],
        configMap,
      });
      ...
      expect(instances.map((instance) => instance.instanceId).toSorted()).toEqual(
        [codexId, claudeId, cursorId, grokId, sigmaId, openCodeId].toSorted(),
      );
      ...
      const sigma = yield* registry.getInstance(sigmaId);
      const openCode = yield* registry.getInstance(openCodeId);
      expect(codex?.driverKind).toBe(codexDriverKind);
      expect(claude?.driverKind).toBe(claudeDriverKind);
      expect(cursor?.driverKind).toBe(cursorDriverKind);
      expect(grok?.driverKind).toBe(grokDriverKind);
      expect(sigma?.driverKind).toBe(sigmaDriverKind);
      expect(openCode?.driverKind).toBe(openCodeDriverKind);
      expect(codex?.displayName).toBe("Codex");
      expect(claude?.displayName).toBe("Claude");
      expect(cursor?.displayName).toBe("Cursor");
      expect(grok?.displayName).toBe("Grok");
      expect(sigma?.displayName).toBe("Sigma");
      ...
      const sigmaSnapshot = yield* sigma!.snapshot.getSnapshot;
      expect(sigmaSnapshot.instanceId).toBe(sigmaId);
      expect(sigmaSnapshot.driver).toBe(sigmaDriverKind);
      expect(sigmaSnapshot.enabled).toBe(false);
      expect(sigmaSnapshot.continuation?.groupKey).toBe(`${sigmaDriverKind}:instance:${sigmaId}`);
