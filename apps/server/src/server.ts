...
const ProviderAuthOperationsLive = ProviderAuthOperations.layer.pipe(
...
const RuntimeCoreFoundationLive = ReactorLayerLive.pipe(
  ...
  Layer.provideMerge(ProviderAuthOperationsLive),
      ...
      Effect.gen(function* () {
        ...
        yield* Effect.forkScoped(
          Effect.sleep("250 millis").pipe(
            ...
            Effect.tap(() => Effect.logInfo("Sigma Connect desired link reconciled on startup")),
            Effect.catch((cause) =>
              Effect.logWarning("Failed to reconcile Sigma Connect desired link on startup", {
                cause,
              }),
            ),
          ),
