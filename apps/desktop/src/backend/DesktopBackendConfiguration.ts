...
const DESKTOP_BACKEND_ENV_NAMES = [
  "SIGMACODE_PORT",
  "SIGMACODE_MODE",
  "SIGMACODE_NO_BROWSER",
  "SIGMACODE_HOST",
  "SIGMACODE_DESKTOP_WS_URL",
  "SIGMACODE_DESKTOP_LAN_ACCESS",
  "SIGMACODE_DESKTOP_LAN_HOST",
...
> {
  ...
  // Build an explicit copy of process.env minus SIGMACODE_HOME (dev-runner
  // exports the Windows-side base dir for the primary; if it leaks into
  // the WSL backend the Linux side ends up sharing C:\Users\...\.sigma\code via
  ...
  for (const [key, value] of Object.entries(process.env)) {
    if (key === "SIGMACODE_HOME") continue;
  ...
  const baseConfig = {
    ...
    // env is already a complete process.env minus SIGMACODE_HOME; pass it
