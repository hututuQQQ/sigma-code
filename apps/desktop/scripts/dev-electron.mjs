...
const remoteDebuggingPort = process.env.SIGMACODE_DESKTOP_REMOTE_DEBUGGING_PORT?.trim();
...
if (devProtocolClient) {
  childEnv.SIGMACODE_DESKTOP_APP_USER_MODEL_ID = devProtocolClient.appBundleId;
  childEnv.SIGMACODE_DESKTOP_PROTOCOL_REGISTRATION_MANAGED = "1";
}
...
function cleanupStaleDevApps() {
  ...
  NodeChildProcess.spawnSync("pkill", ["-f", "--", `--sigmacode-dev-root=${desktopDir}`], {
    stdio: "ignore",
  });
}
...
function startApp() {
  ...
  const launchArgs = devProtocolClient
    ? electronArgs
    : [...electronArgs, `--sigmacode-dev-root=${desktopDir}`, "dist-electron/main.cjs"];
