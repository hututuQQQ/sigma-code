// This file mostly exists because we want dev mode to say "Sigma Code (Dev)" instead of "electron"
...
const devBundleIdSuffix = NodePath.basename(repoRoot)
  .toLowerCase()
  .replaceAll(/[^a-z0-9]+/g, "");
export const APP_DISPLAY_NAME = isDevelopment ? "Sigma Code (Dev)" : "Sigma Code";
export const APP_BUNDLE_ID = isDevelopment
  ? `io.github.hututuqqq.sigmacode.dev.${devBundleIdSuffix || "local"}`
  : "io.github.hututuqqq.sigmacode";
const APP_PROTOCOL_SCHEMES = isDevelopment ? ["sigmacode-dev"] : ["sigmacode"];
...
export function makeDevelopmentLauncherScript({
  ...
  const envEntries = [
    ...
    ["SIGMACODE_PORT", environment.SIGMACODE_PORT],
    ["SIGMACODE_HOME", environment.SIGMACODE_HOME],
    ["SIGMACODE_COMMIT_HASH", environment.SIGMACODE_COMMIT_HASH],
    ["SIGMACODE_OTLP_TRACES_URL", environment.SIGMACODE_OTLP_TRACES_URL],
    ["SIGMACODE_OTLP_EXPORT_INTERVAL_MS", environment.SIGMACODE_OTLP_EXPORT_INTERVAL_MS],
    ["SIGMACODE_DESKTOP_APP_USER_MODEL_ID", APP_BUNDLE_ID],
  ...
  return [
    ...
    `exec ${shellSingleQuote(electronBinaryPath)} --sigmacode-dev-root=${shellSingleQuote(desktopRoot)} ${shellSingleQuote(mainEntryPath)} "$@"`,
    "",
  ].join("\n");
}
...
function buildMacLauncher(electronBinaryPath) {
  ...
  if (isDevelopment) {
    ...
    // Electron.app even though this bundle's Info.plist has the Sigma Code name.
