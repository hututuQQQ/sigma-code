...
const PUBLISHABLE_SIGMA_VERSION_PATTERN = /^\d+\.\d+\.\d+$/u;
...
export function resolveRemoteSigmaCliPackageSpec(input: {
  ...
  if (
    input.isDevelopment ||
    input.updateChannel !== "latest" ||
    !PUBLISHABLE_SIGMA_VERSION_PATTERN.test(appVersion) ||
