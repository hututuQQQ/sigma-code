...
const SSH_ASKPASS_DIR_NAME = "sigmacode-ssh-askpass";
...
# Invoked by ssh via SSH_ASKPASS when Sigma Code re-runs ssh with a cached password
...
printf 'Sigma Code ssh-askpass invoked without T3_SSH_AUTH_SECRET.\\n' >&2
exit 1
`;
...
export const ASKPASS_WINDOWS_SCRIPT = `# Invoked by ssh via SSH_ASKPASS (through ssh-askpass.cmd) when Sigma Code re-runs\r
...
[Console]::Error.WriteLine("Sigma Code ssh-askpass invoked without T3_SSH_AUTH_SECRET.")\r
exit 1\r
`;
...
export const getDefaultSshAskpassDirectory = Effect.fn("ssh/auth.getDefaultSshAskpassDirectory")(
  function* () {
    ...
    const parentDirectory = yield* fs.makeTempDirectory({ prefix: "sigmacode-ssh-runtime-" });
...
> {
  ...
  return {
    ...
    ...(platform === "win32" || baseEnv.DISPLAY || hostDisplay ? {} : { DISPLAY: "sigmacode" }),
  };
});
