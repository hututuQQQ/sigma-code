...
SIGMACODE_PACKAGE_SPEC=@@T3_PACKAGE_SPEC@@
...
if command -v sigma-code >/dev/null 2>&1; then
  exec sigma-code "$@"
fi
if [ -n "$SIGMACODE_PACKAGE_SPEC" ] && command -v npx >/dev/null 2>&1; then
  exec npx --yes "$SIGMACODE_PACKAGE_SPEC" "$@"
fi
if [ -n "$SIGMACODE_PACKAGE_SPEC" ] && command -v npm >/dev/null 2>&1; then
  exec npm exec --yes "$SIGMACODE_PACKAGE_SPEC" -- "$@"
fi
if [ -z "$SIGMACODE_PACKAGE_SPEC" ]; then
  printf 'Remote host is missing the sigma-code CLI. Automatic remote installation is disabled until SIGMACODE_SERVER_NPM_PACKAGE is configured.\\n' >&2
else
  printf 'Remote host is missing the sigma-code CLI and could not install %s because node/npm/npx are unavailable on PATH. Install Node or configure a supported version manager for non-interactive shells.\\n' "$SIGMACODE_PACKAGE_SPEC" >&2
fi
exit 1
`;
...
@@T3_NODE_ENV_SCRIPT@@
STATE_KEY="$1"
STATE_DIR="$HOME/.sigma/code/ssh-launch/$STATE_KEY"
DEFAULT_SERVER_HOME="$HOME/.sigma/code"
DEFAULT_RUNTIME_FILE="$DEFAULT_SERVER_HOME/server-runtime.json"
PORT_FILE="$STATE_DIR/port"
PID_FILE="$STATE_DIR/pid"
MANAGED_FILE="$STATE_DIR/managed"
LOG_FILE="$STATE_DIR/server.log"
RUNNER_FILE="$STATE_DIR/run-sigma-code.sh"
RUNNER_NEXT="$STATE_DIR/run-sigma-code.next.$$"
mkdir -p "$STATE_DIR"
cleanup_runner_next() {
  rm -f "$RUNNER_NEXT"
}
...
if [ -z "$REMOTE_PORT" ]; then
  ...
  nohup env SIGMACODE_NO_BROWSER=1 "$RUNNER_FILE" serve --host 127.0.0.1 --port "$REMOTE_PORT" --base-dir "$DEFAULT_SERVER_HOME" >>"$LOG_FILE" 2>&1 < /dev/null &
  REMOTE_PID="$!"
  printf '%s\\n' "$REMOTE_PID" >"$PID_FILE"
  printf '%s\\n' "$REMOTE_PORT" >"$PORT_FILE"
  printf 'managed\\n' >"$MANAGED_FILE"
  if ! wait_ready "@@T3_READY_TIMEOUT_MS@@"; then
    printf 'Remote Sigma Code server did not become ready on 127.0.0.1:%s.\\n' "$REMOTE_PORT" >&2
...
STATE_DIR="$HOME/.sigma/code/ssh-launch/@@T3_STATE_KEY@@"
DEFAULT_SERVER_HOME="$HOME/.sigma/code"
RUNNER_FILE="$STATE_DIR/run-sigma-code.sh"
mkdir -p "$STATE_DIR"
cat >"$RUNNER_FILE" <<'SH'
