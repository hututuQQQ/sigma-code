...
- In a linked git worktree, dev state defaults to that worktree's gitignored `.sigma-code`. This deliberately outranks an ambient `SIGMACODE_HOME`, which could otherwise select the installed app's live `~/.sigma/code` database. An explicit `--home-dir` still wins.
