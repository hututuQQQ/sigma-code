...
export const DEFAULT_T3_HOME = Effect.map(Effect.service(Path.Path), (path) =>
  path.join(NodeOS.homedir(), ".sigma", "code"),
);
...
/**
 * `role` matters because only the backend honours `--host`/`SIGMACODE_HOST`; the
...
export class DevRunnerInvalidPortOffsetError extends Schema.TaggedErrorClass<DevRunnerInvalidPortOffsetError>()(
  ...
  {
    configKey: Schema.Literal("SIGMACODE_PORT_OFFSET"),
...
const OffsetConfig = Config.all({
  portOffset: optionalIntegerConfig("SIGMACODE_PORT_OFFSET"),
  devInstance: optionalStringConfig("SIGMACODE_DEV_INSTANCE"),
});
...
> {
  if (config.portOffset !== undefined) {
    if (config.portOffset < 0) {
      return Effect.fail(
        new DevRunnerInvalidPortOffsetError({
          configKey: "SIGMACODE_PORT_OFFSET",
    ...
    return Effect.succeed({
      ...
      source: `SIGMACODE_PORT_OFFSET=${config.portOffset}`,
    });
  }
  ...
  if (seed) {
    if (/^\d+$/.test(seed)) {
      return Effect.succeed({
        offset: Number(seed),
        source: `numeric SIGMACODE_DEV_INSTANCE=${seed}`,
      });
    }

    const offset = ((Hash.string(seed) >>> 0) % MAX_HASH_OFFSET) + 1;
    return Effect.succeed({ offset, source: `hashed SIGMACODE_DEV_INSTANCE=${seed}` });
  }
...
export function createDevRunnerEnv({
  ...
  return Effect.gen(function* () {
    ...
    // Precedence (--home-dir > worktree .sigma-code > ambient SIGMACODE_HOME) is resolved
    ...
    if (configuredBaseDir !== undefined) {
      output.SIGMACODE_HOME = resolvedBaseDir;
    } else {
      delete output.SIGMACODE_HOME;
    }

    if (!isDesktopMode) {
      output.SIGMACODE_PORT = String(serverPort);
      ...
      if (mode === "dev" || mode === "dev:web") {
        ...
        output.SIGMACODE_SINGLE_ORIGIN_DEV = "1";
      } else {
        output.VITE_HTTP_URL = `http://localhost:${serverPort}`;
        output.VITE_WS_URL = `ws://localhost:${serverPort}`;
        delete output.SIGMACODE_SINGLE_ORIGIN_DEV;
      }
    } else {
      output.SIGMACODE_PORT = String(serverPort);
      ...
      delete output.SIGMACODE_SINGLE_ORIGIN_DEV;
      delete output.SIGMACODE_MODE;
      delete output.SIGMACODE_NO_BROWSER;
      delete output.SIGMACODE_HOST;
    }

    if (!isDesktopMode && host !== undefined) {
      output.SIGMACODE_HOST = host;
    }

    if (!isDesktopMode) {
      output.SIGMACODE_NO_BROWSER = browser === true ? "0" : "1";
    }

    if (autoBootstrapProjectFromCwd !== undefined) {
      output.SIGMACODE_AUTO_BOOTSTRAP_PROJECT_FROM_CWD = autoBootstrapProjectFromCwd ? "1" : "0";
    } else {
      delete output.SIGMACODE_AUTO_BOOTSTRAP_PROJECT_FROM_CWD;
    }

    if (logWebSocketEvents !== undefined) {
      output.SIGMACODE_LOG_WS_EVENTS = logWebSocketEvents ? "1" : "0";
    } else {
      delete output.SIGMACODE_LOG_WS_EVENTS;
    }

    if (mode === "dev") {
      output.SIGMACODE_MODE = "web";
      delete output.SIGMACODE_DESKTOP_WS_URL;
    }
 ...
 * target reach it there. When `--host`/`SIGMACODE_HOST` moves the backend onto
...
export function runDevRunnerWithInput(input: DevRunnerCliInput) {
  return Effect.gen(function* () {
      ...
      Effect.mapError(
        (cause) =>
          new DevRunnerConfigurationError({
            configKeys: ["SIGMACODE_PORT_OFFSET", "SIGMACODE_DEV_INSTANCE"],
            cause,
          }),
      ),
    );
    ...
    // (gitignored) `.sigma-code` — see @t3tools/shared/devHome for why this must
    // outrank an ambient SIGMACODE_HOME. `--home-dir` still wins.
    ...
    const resolvedT3Home =
      ...
      worktreeHome ??
      (hostEnvironment.SIGMACODE_HOME?.trim() || undefined);
    ...
    const selectionSuffix =
      serverOffset !== offset || webOffset !== offset
        ...
        : "";
    const baseDir = env.SIGMACODE_HOME ?? (yield* DEFAULT_T3_HOME);

    yield* Effect.logInfo(
      `[dev-runner] mode=${input.mode} source=${source}${selectionSuffix} serverPort=${String(env.SIGMACODE_PORT)} webPort=${String(env.PORT)} baseDir=${baseDir}`,
    );
    ...
    if (input.share) {
      if (input.mode === "dev:server") {
        ...
        if (shared) {
          ...
          env.SIGMACODE_DEV_ALLOWED_ORIGINS = [
            env.SIGMACODE_DEV_ALLOWED_ORIGINS,
...
const devRunnerCli = Command.make("dev-runner", {
  ...
  t3Home: Flag.string("home-dir").pipe(
    Flag.withDescription(
      "Explicit Sigma Code state directory (equivalent to SIGMACODE_HOME). Inside a git worktree this defaults to that worktree's own .sigma-code directory.",
    ),
    Flag.optional,
  ...
  autoBootstrapProjectFromCwd: Flag.boolean("auto-bootstrap-project-from-cwd").pipe(
    Flag.withDescription(
      "Auto-bootstrap toggle (equivalent to SIGMACODE_AUTO_BOOTSTRAP_PROJECT_FROM_CWD).",
    ),
    Flag.withFallbackConfig(optionalBooleanConfig("SIGMACODE_AUTO_BOOTSTRAP_PROJECT_FROM_CWD")),
  ),
  logWebSocketEvents: Flag.boolean("log-websocket-events").pipe(
    Flag.withDescription("WebSocket event logging toggle (equivalent to SIGMACODE_LOG_WS_EVENTS)."),
    Flag.withAlias("log-ws-events"),
    Flag.withFallbackConfig(optionalBooleanConfig("SIGMACODE_LOG_WS_EVENTS")),
  ),
  host: Flag.string("host").pipe(
    Flag.withDescription("Server host/interface override (forwards to SIGMACODE_HOST)."),
    Flag.withFallbackConfig(optionalStringConfig("SIGMACODE_HOST")),
  ),
  port: Flag.integer("port").pipe(
    Flag.withSchema(Schema.Int.check(Schema.isBetween({ minimum: 1, maximum: 65535 }))),
    Flag.withDescription("Server port override (forwards to SIGMACODE_PORT)."),
    Flag.withFallbackConfig(optionalPortConfig("SIGMACODE_PORT")),
  ),
